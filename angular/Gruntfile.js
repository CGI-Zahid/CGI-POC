...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        cssmin : {
            dist : {
                files : {
                    '<%= yeoman.dist %>/styles/cgipoc.css' : [ '.tmp/styles/{,*/}*.css' ]
                }
            }
        },
        uglify : {
                ...
                files : [
                        {
                            ...
                            src : [ '*.{ico,png,txt}', '.htaccess', '*.html', 'views/{,*/}*.html',
                                    'images/{,*/}*.{webp}', 'styles/fonts/{,*/}*.*', 'language/*.json',
                                    'uswds-0.14.0/**/*.*' ]
                        }, {
                            expand : true,
                            cwd : '.tmp/images',
    ...
    grunt.registerTask('serve', 'Compile then start a connect web server', function(target) {
        ...
        grunt.task.run([ 'clean:server', 'wiredep', 'sass', 'concurrent:server', 'autoprefixer:server',
    ...
    grunt.registerTask('test',
                       [ 'clean:server', 'wiredep', 'concurrent:test', 'autoprefixer', 'connect:test', 'karma' ]);

    grunt.registerTask('build', [ 'clean:dist', 'wiredep', 'sass', 'useminPrepare', 'concurrent:dist', 'autoprefixer',
